...
class KernelModuleGUI:
  ...
  def __init__(self, root):
    self.root = root
    self.root.title("Linux Kernel Module Inspector (Themed)")
  ...
  def setup_ui(self):
    ...
    # --- Top Dashboard / Header Frame (Permanent Counter & Theme Toggles) ---
    ...
    # --- Search Filter & Category Theme Bar ---
    ...
    # Theme / Category Filter Dropdown
    ttk.Label(
        filter_bar, text="🏷️ Functional Theme:", font=("Segoe UI", 10, "bold")
    ).pack(side=tk.LEFT, padx=(0, 6))
    
    self.category_var = tk.StringVar(value="All Themes")
    categories = [
        "All Themes",
        "Audio / Sound",
        "Network / Wireless",
    ...
    self.tree.heading(
        "category", text="Theme Category", command=lambda: self.sort_by("category")
    )
    self.tree.heading(
  ...
  def classify_module(self, name):
    """Auto-categorizes a module into a functional theme based on common naming conventions."""
  ...
  def filter_modules(self, *args):
    ...
    for mod in self.modules_data:
      ...
      match_category = (selected_category == "All Themes" or mod["category"] == selected_category)
      # Match search text filter
    ...
    if query or selected_category != "All Themes":
