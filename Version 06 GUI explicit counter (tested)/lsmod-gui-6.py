...
class KernelModuleGUI:
  ...
  def setup_ui(self):
    ...
    # --- Top Dashboard / Header Frame (Permanent Counter & Toolbar) ---
    dash_frame = ttk.Frame(self.main_frame)
    dash_frame.pack(fill=tk.X, pady=(0, 10))

    # Stats / Counter Label (Always Visible Overview)
    ...
    ttk.Label(filter_bar, text="🔍 Search Filter:", font=("Segoe UI", 10, "bold")).pack(
        side=tk.LEFT, padx=(0, 6)
    )
    self.search_var = tk.StringVar()
    self.search_var.trace_add("write", self.filter_modules)
    self.search_entry = ttk.Entry(
        filter_bar, textvariable=self.search_var, width=32, font=("Segoe UI", 10)
    )
    ...
    self.filter_status_lbl.pack(side=tk.LEFT)
    ...
    # --- Bottom Status Bar (Action Feedback Only) ---
  ...
  def load_modules(self):
    ...
    try:
      ...
      self.filter_modules()
      self.status_var.set(f"✨ Successfully refreshed active modules list.")
  ...
  def filter_modules(self, *args):
    ...
    if query:
      self.filter_status_lbl.config(text=f"(Showing {count} of {len(self.modules_data)} matching filter)")
    else:
